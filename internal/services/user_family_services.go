...
func (s *userFamilyService) Create(req request.UserFamilyRequest, actorID uint) (response.UserFamilyResponse, error) {
	...
	for _, member := range req.Member {
		userFamily := &models.UserFamily{
			...
			Count:    req.Count,
			Member:   member,
			CreateBy: fmt.Sprint(actorID),
			ModifyBy: fmt.Sprint(actorID),
		}
...
func (s *userFamilyService) GetAll() ([]response.UserFamilyResponse, error) {
	...
	for _, fam := range families {
		...
		if entry, exists := grouped[fam.UserID]; exists {
			entry.Members = append(entry.Members, fam.Member)
			entry.Count += 1
		} else {
			grouped[fam.UserID] = &response.UserFamilyResponse{
				UUID:    fam.UUID, // could use latest or first UUID
				Status:  fam.Status,
				Count:   1,
				Members: []string{fam.Member},
				User: struct {
...
func (s *userFamilyService) GetByID(id uint) (response.UserFamilyResponse, error) {
	...
	for _, f := range families {
		grouped.Members = append(grouped.Members, f.Member)
	}
...
func (s *userFamilyService) GetByUserUUID(userUUID string) ([]response.UserFamilyResponse, error) {
	...
	for _, fam := range families {
		grouped.Members = append(grouped.Members, fam.Member)
	}
...
func (s *userFamilyService) Update(userUUID string, req request.UserFamilyRequest, actorID uint) (response.UserFamilyResponse, error) {
	// Fetch the original record to find the associated user
	existing, err := s.repo.GetByUUID(userUUID)
	if err != nil {
		return response.UserFamilyResponse{}, err
	}

	// Delete all existing family rows for that user
	if err := s.repo.DeleteByUserID(existing.UserID); err != nil {
		return response.UserFamilyResponse{}, err
	}

	// Re-insert with updated members
	for _, member := range req.Member {
		newFam := &models.UserFamily{
			UUID:     uuid.NewString(),
			UserID:   existing.UserID,
			Status:   req.Status,
			Count:    uint(len(req.Member)),
			Member:   member,
			CreateBy: fmt.Sprint(actorID),
			ModifyBy: fmt.Sprint(actorID),
		}
	...
	// Re-fetch all user family rows to group response
	families, err := s.repo.GetByUserUUID(existing.User.UUID)
...
func (s *userFamilyService) Delete(uuid string) (response.UserFamilyResponse, error) {
	...
	// Re-fetch remaining family members
	families, err := s.repo.GetByUserUUID(fam.User.UUID)
	if err != nil {
		return response.UserFamilyResponse{}, err
	}

	// If none left, return empty response with user info
	if len(families) == 0 {
		return response.UserFamilyResponse{
			UUID:    fam.UUID,
			Status:  fam.Status,
			Count:   0,
			Members: []string{},
